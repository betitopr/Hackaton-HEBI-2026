...
columns = ['ts', 'acc_x', 'acc_y', 'acc_z', 'gyr_x', 'gyr_y', 'gyr_z', 'q_w', 'q_x', 'q_y', 'q_z']
...
gravity_world = np.mean(accel_world[:100], axis=0) # Asumimos reposo inicial para calibrar
...
plt.subplot(3, 1, 1)
plt.plot(df['time_sec'], df['acc_y'], color='gray', alpha=0.5, label='Original (Body Y)')
...
plt.subplot(3, 1, 3)
plt.plot(df['time_sec'], df['linear_mag'], color='black', label='Magnitud Movimiento Puro')
plt.ylabel('m/s²')
plt.xlabel('Tiempo (s)')
plt.legend()
plt.grid(True)
